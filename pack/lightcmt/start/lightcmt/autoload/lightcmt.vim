...
function lightcmt#CommentToggle() range abort
    try
        let l:comment_mark_left = s:delimiterMap[&filetype]['left']
        let l:comment_mark_left_escape = escape(l:comment_mark_left, s:escale_char)
        let l:comment_mark_left_escape_slash = escape(l:comment_mark_left, '/&')
        if has_key(s:delimiterMap[&filetype], 'right')
            let l:comment_mark_right = s:delimiterMap[&filetype]['right']
            let l:comment_mark_right_escape = escape(l:comment_mark_right, s:escale_char)
...
function lightcmt#CommentTail() abort
    try
        let l:comment_mark_left = s:delimiterMap[&filetype]['left']
        if !has_key(s:delimiterMap[&filetype], 'right')
            silent! execute 'normal! $a ' . l:comment_mark_left
            startinsert!
        else
            let l:comment_mark_right = s:delimiterMap[&filetype]['right']
            let l:right_length = len(l:comment_mark_right)
            silent! execute 'normal! $a ' . l:comment_mark_left . l:comment_mark_right
            call cursor('.', col('.') - l:right_length + 1)
            startinsert
        endif
    catch /.*/
    endtry
